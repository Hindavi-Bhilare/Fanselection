...
public class VectorMaths {
	...
	public void setVector(double[] coord0, double[] coord1) {
		...
		if (n != this.nDimensionsEntered)
			throw new IllegalArgumentException("The two dimensions, " + this.nDimensionsEntered + " and " + n + ", should be equal");
		if (this.nDimensionsEntered > 3)
			throw new IllegalArgumentException("VectorMaths will not handle dimensional spaces greater than 3-dimensional");
	...
	// Instance method - returns true if the two vectors are identical - false
	// if they are not
	...
	// Test for identical vectors
	// Static method - returns true if the two vectors are identical - false if
	// they are not
	...
	public Phasor toPhasor() {
		...
		case 1:
			mag = Math.abs(co1[0] - co0[0]);
			phase = 0.0;
			break;
		case 2:
			mag = Point.distance(this.point0, this.point1);
